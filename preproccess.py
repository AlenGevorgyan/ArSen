...
actions = np.array(['me', 'you', "նա", "մենք", "my", "քո", "նրա", "yourself", "ո՞վ", "ի՞նչ", "why՞", "որովհետև", "ոչինչ", "միշտ", "ուր", "որտե՞ղ", "որտեղի՞ց", "when", "all", "հազվագյուտ", "բարև ձեզ", "ցտեսություն", "ողջույն", "հրավիրել", "ներողություն", "ask", "ծանոթանալ", "ընկեր", "անուն", "ազգանուն", "հայրանուն", "խուլ", "թույլ լսող", "ժեստ", "կոչվել"]) #"մայր", "հայր", "14"
print(len(actions))
...
PATH = os.path.join('mer_data')

# Create directories for each action, sequence, and frame in the dataset
for action, sequence in product(actions, range(sequences)):
    try:
        # NOTE: The original script creates sequence directories, not frame directories, but that's fine.
    ...
    for action, sequence, frame in product(actions, range(sequences), range(frames)):
        ...
        if frame == 0:
            while True:
                ...
                # FIX 1: Create a writeable copy of the image before drawing on it
                image = image.copy()
                ...
                if cv2.getWindowProperty('Camera', cv2.WND_PROP_VISIBLE) < 1:
                    # Release the camera and close all windows before exiting outer loop
                    cap.release()
            ...
            # FIX 2: Create a writeable copy of the image before drawing on it
            image = image.copy()
        ...
        # Extract the landmarks from pose and both hands and save them in arrays
        keypoints = keypoint_extraction(results)
        frame_path = os.path.join(PATH, action, str(sequence), f"frame_{frame:05d}.npy")
